...
const groupMessagesByMinuteAndUserId = (messages: Message[]): { messages: Message[]; user_id: number; user_name: string ;minute: string }[] => {
    const groupedMessages: { user_id: number; messages: Message[]; user_name: string ; minute: string }[] = [];
    let currentGroup: { user_id: number; messages: Message[]; user_name: string ;minute: string } | null = null;
    ...
    messages.forEach(message => {
      ...
      if (!currentGroup || currentGroup.user_id !== message.user_id || currentGroup.minute !== minuteKey) {
        currentGroup = { user_id: message.user_id, messages: [], minute: minuteKey, user_name: message.user_name };
