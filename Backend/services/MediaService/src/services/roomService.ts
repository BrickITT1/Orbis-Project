...
export const roomService = {
  async addPeerToRoom(
    ...
    await redisClient.sAdd(usersSetKey, String(userId));

    // Добавляем peerId в список пиров пользователя
    await redisClient.sAdd(userPeersKey, String(peerId));
  ...
  async getRoomsByPeer(peerId: string): Promise<string[]> {
    const rooms = await redisClient.sMembers(`peer:${peerId}:rooms`);
    return rooms ?? [];
  },

  async editPeer(roomId: string, peerId: string, key: string, value: string) {
    if (!roomId || !peerId || !key || !String(value)) {
      console.log(
        `Значения roomId: ${roomId}, peerId: ${peerId}, key: ${key}, value: ${value}`
      )
      throw new Error("Отсутствуют обязательные параметры");
    }
    ...
    let peerData;
    try {
      peerData = JSON.parse(peerRaw);
    } catch (err) {
      throw new Error("Ошибка при разборе данных пира");
    }

    if (!(key in peerData)) {
      throw new Error(`Ключ "${key}" не найден в данных пира`);
    }

    peerData[key] = value;

    // Сохраняем обратно в Redis
    await redisClient.hSet(peersKey, peerId, JSON.stringify(peerData));
    console.log(`Поле "${key}" пира "${peerId}" обновлено на значение: ${value}`);
  }
