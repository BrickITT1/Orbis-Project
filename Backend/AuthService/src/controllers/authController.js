...
export const sendCodeCheck = async (req, res) => {
  try {
...
export const register = async (req, res) => {
  ...
  try {
    const { email, password, display_name, username, birth_date } = req.body;
...
export const login = async (req, res) => {
  const client = await pool.connect();
    ...
    try {
      ...
      const accessToken = jwt.sign({ id: user.rows[0].id }, process.env.ACCESS_TOKEN_SECRET, {
        expiresIn: "15m",
      });
  
      const refreshToken = jwt.sign({ id: user.rows[0].id }, process.env.REFRESH_TOKEN_SECRET, {
        expiresIn: "7d",
      });
      ...
      res.json({ access_token: accessToken, username: user.rows[0].username, info: userInformation.rows[0] });
    } catch (error) {
      console.log(error)
      res.status(500).json({ error: "Internal server error" });
    } finally {
